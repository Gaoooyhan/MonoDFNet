...
class _predictor(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.aggregate_reg = cfg.AGGREGATE_REG
        ...
        if self.pred_ground_plane:
            ...
            self.horizon_head[-1].bias.data.fill_(- np.log(1 / cfg.MODEL.HEAD.INIT_P - 1))
    ...
    def forward(self, features, targets=None):
        ...
        if self.pred_ground_plane:
            feature_horizon = self.horizon_head[:-1](features)
            output_horizon = self.horizon_head[-1](feature_horizon)
            output_horizon = sigmoid_hm(output_horizon)
        ...
        if self.pred_ground_plane:
            return {'cls': output_cls, 'reg': output_regs, 'horizon': output_horizon}
